...
func (r *TodoItemPostgres) Create(listId int, item todo_app.TodoItem) (int, error){
	...
	createListItemQuery := fmt.Sprintf("INSERT INTO %s(list_id, item) values ($1, $2)", listsItemsTable)
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo_app.UpdateItemInput) error{
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul WHERE ti.id = li.item_id AND li.list_id AND ul.user_id = $%id AND ti.id = $%id`,
